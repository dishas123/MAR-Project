...
class FrontierExplorer(Node):
    ...
    def __init__(self):
        ...
        self.failed_goals = []      # remember failed goals to skip them
    ...
    def explore(self):
        ...
        # Try frontiers from largest to smallest, skip previously failed ones
        frontiers_sorted = sorted(frontiers, key=len, reverse=True)
        goal_cell = None

        for cluster in frontiers_sorted:
            rows = [c[0] for c in cluster]
            cols = [c[1] for c in cluster]
            cx, cy = self.cell_to_world(int(np.mean(rows)), int(np.mean(cols)))

            # Skip if this goal is too close to a previously failed goal
            too_close = False
            ...
            # Skip if goal cell is not clearly free
            center_row = int(np.mean(rows))
            center_col = int(np.mean(cols))
            if not self.is_safe_cell(center_row, center_col):
                continue

            if not too_close:
                goal_cell = (center_row, center_col)
                self.current_goal = (cx, cy)
                break

        if goal_cell is None:
            self.get_logger().warn('All frontiers are near failed goals, clearing memory...')
            self.failed_goals = []
            return

        world_x, world_y = self.cell_to_world(*goal_cell)
        self.get_logger().info(f'Sending goal: ({world_x:.2f}, {world_y:.2f})')
        self.send_nav_goal(world_x, world_y)

    def is_safe_cell(self, row, col):
        """Check that the cell and its neighbors are free (not obstacle or unknown)."""
        h, w = self.map_data.shape
        margin = 3  # cells around goal must be free
        r0 = max(0, row - margin)
        r1 = min(h, row + margin)
        c0 = max(0, col - margin)
        c1 = min(w, col + margin)
        region = self.map_data[r0:r1, c0:c1]
        # If any cell in margin is an obstacle, skip this goal
        if np.any(region > 50):
            return False
        return True
    ...
    def goal_response_cb(self, future):
        ...
        if not self.current_goal_handle.accepted:
            self.get_logger().warn('Goal rejected — adding to failed list')
